``` c++
AddArgument(string, logFile).Group("Logging").Name("log_file").Short("l")
        .ArgumentType("FILE").Description("Redirect logs to this file")
        .DefaultValue("", "No logs").ImplicitValue("");
...
AddArgument(bool, logInfo).Group("Logging").Name("info")
        ...
        .Description("Log info as well?");

AddArgument(bool, dieOnErrors).Group("Logging").Name("no_die_on_error")
        .DefaultValue(1).ImplicitValue(0)
        .Description("Continue program if an error rises");
```
